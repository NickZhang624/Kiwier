<?php 
/*
$count：display total posts
$page_size：display how many posts in cur page
$num_btn：display how many btns in cur page
*/
function page($count,$page_size,$num_btn=10,$page='page'){
	...
	$limit="limit {$start},{$page_size}";//push to Mysql

	$current_url=$_SERVER['REQUEST_URI'];//get cur url path
	$arr_current=parse_url($current_url);//split url into an array
	$current_path=$arr_current['path'];//save 'path' 
	$url='';
	if(isset($arr_current['query'])){
		parse_str($arr_current['query'],$arr_query);
		unset($arr_query[$page]);//remove data
		if(empty($arr_query)){
			$url="{$current_path}?{$page}=";
		}else{
			$other=http_build_query($arr_query);//rebuild other data
	...
	if($num_btn>=$page_num_all){
		...
		if(count($html)>=3){
			...
			$key_end=key($html);
			if($key_first!=1){
				//shift means remove first data from an array
				array_shift($html);
				//unshift means insert one or more into arrays and put to first position
				array_unshift($html,"<a href='{$url}1'>1...</a>");
			}
			if($key_end!=$page_num_all){
				//pop means remove the last data from an array
				array_pop($html);
